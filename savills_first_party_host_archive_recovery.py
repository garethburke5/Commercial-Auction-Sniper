...
for prefix in PREFIXES:
    st,final,rows=cdx(prefix); queries.append({'prefix':prefix,'status':st,'records':len(rows),'final_url':final}); all_rows.extend(rows); time.sleep(.6)
uniq={}
for r in all_rows:
    u=html.unescape((r.get('original') or '').strip())
    if u: uniq.setdefault(u,r)

# Prefer property/detail-looking URLs but retain catalogue pages for date/lot evidence.
def score(u):
    low=u.lower(); s=0
    if '/component/bidding/' in low: s+=10
    if '/auctions/' in low: s+=5
    if re.search(r'-\d{2,6}(?:\?|$)',low): s+=4
    if any(k in low for k in ('page-','quantity-','sort-by','property_type')): s-=3
    return s

rows_sorted=sorted(uniq.items(),key=lambda kv:(-score(kv[0]),kv[0]))
checks=[]; strict=[]
for original,r in rows_sorted[:1800]:
    ts=r.get('timestamp') or ''
    archive=f'https://web.archive.org/web/{ts}id_/{original}' if ts else original
    st,final,text=get(archive,timeout=22)
    if st!=200:
        checks.append({'original_url':original,'archive_url':archive,'status':st}); continue
    plain=strip_text(text)
    dates=[]
    for d,m in DATE_RE.findall(plain):
        dates.append(f'2018-{MONTHS[m.title()]:02d}-{int(d):02d}')
    ...
    checks.append(rec)
    # Strict first-party candidate requires one explicit 2018 auction date, lot, postcode and commercial signal.
    if len(dates)==1 and lots and pcs and commercial:
        strict.append({'auction_date':dates[0],'lot_number':lots[0],'postcodes':pcs[:3],'title':title,'original_url':original,'archive_url':archive,'snippet':plain[:1200]})
    time.sleep(.06)

# Deduplicate strict candidates by event identity evidence.
seen=set(); dedup=[]
for x in strict:
...
run={'at':now,'route':'savills-2018-first-party-savills-host-wayback-enumeration','target_year':2018,'cdx_queries':queries,'unique_first_party_urls':len(uniq),'archive_surfaces_replayed':len(checks),'strict_first_party_candidates':len(dedup),'strict_candidates':dedup[:80],'canonical_events_added':0}
source['savills_year_gap_savills_host_last_run']=run
if dedup:
    detail=f'Recovered {len(dedup)} first-party Savills archived pages with explicit 2018 date + lot + postcode + commercial evidence. They still require one-to-one reconciliation against the surviving PropertyAuctions AID/date/lot tuples before canonical promotion.'
    ...
    detail=f'Enumerated {len(uniq)} archived first-party auctions.savills.co.uk URLs from 2017-2019 and replayed {len(checks)} surfaces, but none produced a strict 2018 date + lot + postcode + commercial evidence bundle.'
    nxt='Enumerate first-party Savills PDF/brochure/static-asset namespaces and query archived document filenames around the surviving 2018 auction dates, then reconcile exact lot/address evidence to the AID manifest.'

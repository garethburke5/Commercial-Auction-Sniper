...
def _extract_targets(s, found):
    ...
    for a in s.find_all("a", href=True):
        ...
        if not _card_is_target(card): continue
...
def _discover():
    ...
    for template in SEARCHES:
        ...
        for page in range(1, 31):
            ...
            page_found = {}; _extract_targets(s, page_found)
...
def _hydrate(item):
    ...
    if isinstance(meta, dict):
        card = meta.get("card") or ""; teaser_image = meta.get("image"); teaser_date = meta.get("auction_date")
    else: card, teaser_image, teaser_date = meta, None, None
    try: s = soup(url, use_browser=False)
    except Exception:
        try: s = soup(url, use_browser=True)
        except Exception: return _teaser_lot(url, card, teaser_image, teaser_date)
    ...
    if not address: return _teaser_lot(url, card, detail_image, teaser_date)
    ...
    if not is_commercial(combined): return None
    ...
    occupation = None
    if re.search(r"\bvacant\b|vacant possession", combined, re.I) and not rent: occupation = "Vacant / vacant possession"
...
def collect():
    try:
        ...
        if not live_targets:
            return SourceResult(SOURCE, "CATALOGUE PENDING", [], f"Allsop exposed {len(targets)} commercial/mixed-use history/featured lot pages, but no current/future commercial catalogue lots are published yet.", discovered_count=len(targets), authoritative_snapshot=True)
        lots=[]; failures=0
        ...
        return SourceResult(SOURCE,status,lots,f"Allsop canonical+search collector: {len(live_targets)} current/future commercial/mixed-use candidate pages from {len(targets)} total target tiles; {len(lots)} published; {failures} detail failures.",discovered_count=len(live_targets),authoritative_snapshot=False,scope_dates=tuple(sorted({x.auction_date for x in lots if x.auction_date})))

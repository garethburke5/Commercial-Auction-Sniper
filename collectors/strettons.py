...
def collect():
    try:
        ...
        try:
            ...
            s = soup(COMMERCIAL, use_browser=True)
        ...
        for a in s.find_all("a", href=True):
            ...
            if not any(x in low for x in ("auction-commercial-property-for-sale", "auction-mixed-use-property-for-sale")):
                continue
            card = nearest_card(a, 2600) or norm(a.get_text(" ", strip=True))
            lot_no = _lot_no(card)
            if not lot_no:
                continue
            targets.setdefault(href, (card, lot_no))
        ...
        for href, (card, lot_no) in targets.items():
            ...
            try:
                lot = detail_lot(
                    ...
                    use_browser=False, suppress_prior=True,
                )
            except Exception:
                try:
                    lot = detail_lot(
                        SOURCE, href, seed=card, lot_number=lot_no,
                        auction_date=auction_date, force_commercial=True,
                        use_browser=True, suppress_prior=True,
                    )
        ...
        if not lots:
            return SourceResult(
                SOURCE, "FAILED", [],
                f"Discovered current auction {auction_date}, but no commercial detail pages published.",

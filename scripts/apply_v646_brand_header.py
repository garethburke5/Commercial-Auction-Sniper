...
for old in ('BUILD = "V6.45-YIELD-LABEL-RIGHT"','BUILD = "V6.44-ALIGNED-DETAILS"'):
    if old in s:
        s=s.replace(old,'BUILD = "V6.46-BRAND-MASTHEAD"',1)
        break
css='''
/* V6.46 stronger product masthead */
...
.badge{font-size:.66rem!important;padding:7px 11px!important;box-shadow:inset 0 0 0 1px rgba(129,236,170,.05)!important}
@media(max-width:650px){.hero{min-height:70px!important;padding:12px 13px 11px 16px!important}.hero:before{width:4px}.brand{font-size:1.62rem!important}.tagline{font-size:.61rem!important;margin-top:6px!important}.sub{font-size:.42rem!important}.badge{font-size:.50rem!important;padding:5px 7px!important}}
'''
s=s.replace('</style>\n""",unsafe_allow_html=True)',css+'</style>\n""",unsafe_allow_html=True)',1)
p.write_text(s,encoding='utf-8')
py_compile.compile(str(p),doraise=True)
print('V6.46 stronger Auction Sniper masthead applied')

...
def collect(max_guide=300000):
    try:
        ...
        for a in soup.find_all("a",href=True):
            ...
            if not any(x in low for x in ["retail property","commercial property","mixed use","mixed-use","commercial building","retail unit","commercial unit"]): continue
            g=parse_guide(card)
        ...
        for u,card,lotno in candidates:
            try:
                lot=detail_lot(NAME,u,seed_text=card,lot_number=lotno,force_commercial=True,property_type="AHL Commercial Feed")
                if lot.guide_price is None or lot.guide_price<=max_guide: lots.append(lot)
            except Exception: pass
        return CollectorResult(NAME,"OK",lots,f"Dedicated commercial page; {len(candidates)} candidate cards")

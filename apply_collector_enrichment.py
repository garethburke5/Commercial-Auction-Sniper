...
new='''    low=text.lower()\n    row=dict(\n        source=source,lot=lot,date=date,address=address,guide=guide,\n        rent=parse_rent(text),\n        tenure=("Freehold" if "freehold" in low[:1800]\n                else "Leasehold" if "leasehold" in low[:1800] else None),\n        vat="UNKNOWN",url=url,desc=text[:1000],image=image\n    )\n    # Exact-page enrichment repairs polluted regional titles and captures the\n    # investment particulars already published by Auction House.\n    return merge_enrichment(row,extract_particulars(page_html,source,url))\n'''
assert old in s
s=s.replace(old,new,1)
...
# Enrich AHL catalogue rows from their exact pages. This is deliberately bounded to
# the current commercial catalogue and runs concurrently so richer facts do not make
# the board unusably slow.
...
# Enrich Bond Wolfe rows after its existing collector has resolved exact lot URLs.
needle='''        ("Bond Wolfe",_bond_wolfe_current),\n'''
# Function itself is elsewhere; wrap centrally so any Bond Wolfe implementation gets exact-page facts.
# Add helper immediately before _merge_catalogue_rows.

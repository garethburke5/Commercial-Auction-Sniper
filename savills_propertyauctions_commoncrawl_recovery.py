...
# Bounded but spread across the 2018 crawl calendar. This avoids a stalled run while still
# testing materially different historical snapshots for every master-manifest auction.
INDEXES = [
    'CC-MAIN-2018-51', 'CC-MAIN-2018-43', 'CC-MAIN-2018-34',
    'CC-MAIN-2018-22', 'CC-MAIN-2018-13', 'CC-MAIN-2018-05',
]
...
def query_index(session, index, target):
    ...
    r = session.get(url, params={'url': target, 'output': 'json', 'filter': 'status:200'}, timeout=(3, 6))
...
def fetch_warc(session, rec):
    ...
    if not fn or off is None or ln is None:
        return None, 'missing warc coordinates'
    start = int(off); end = start + int(ln) - 1
    r = session.get('https://data.commoncrawl.org/' + fn, headers={'Range': f'bytes={start}-{end}'}, timeout=(4, 12))
    ...
    if idx < 0:
        idx = low.find(b'<!doctype html')
    if idx < 0:
        return None, 'no html payload marker'
...
def main():
    ...
    master_dates = {str(x)[:10] for x in (gap.get('master_manifest_dates') or []) if x}
    all_cats = gap.get('revalidated_propertyauctions_catalogues') or []
    cats = [c for c in all_cats if str(c.get('date') or '')[:10] in master_dates]
    clues = gap.get('clue_runs') or []
    clue_keys = {
        (str(c.get('auction_date') or '')[:10], norm_lot(c.get('lot_number')))
        for c in clues if c.get('auction_date') and c.get('lot_number')
    }
                    ...
                    for lot in parsed['lots']:
                        if (date, lot) in clue_keys and parsed['postcodes'] and parsed['commercial_signal']:
                            deterministic.append({
                                ...
                                'aid': aid,
                                'lot_number': lot,
                                'postcodes': parsed['postcodes'][:8],
                                'commoncrawl_index': rec.get('index'),
    ...
    diag = {
        ...
        'master_manifest_dates_targeted': sorted(master_dates),
        'catalogues_attempted': len(runs),
        'commoncrawl_indexes_per_catalogue': len(INDEXES),
    ...
    if deterministic:
        msg = (
            f"Common Crawl found {len(records)} indexed records for the {len(runs)} master-manifest 2018 Savills catalogues, "
            f"fetched {len(fetched)} WARC records and recovered {len(deterministic)} deterministic date+lot+postcode+commercial identity candidates; "
            "no row is promoted until full-address/result reconciliation is complete."
        )
        ...
        msg = (
            f"After Wayback transport timeouts, bounded Common Crawl fallback checked {len(runs)} master-manifest 2018 Savills catalogues across {len(INDEXES)} 2018 indexes each, "
            f"found {len(records)} indexed records and fetched {len(fetched)} WARC records, but recovered 0 deterministic date+lot+postcode+commercial identities."
        )
    ...
    state['savills_year_gap_last_blocker'] = {
        'at': at,
        'route': diag['route'],
        'failing_url_or_route': runs[0]['target'] if runs else 'no master-manifest 2018 catalogue available',
        'message': msg,
    ...
    diagnostic['commoncrawl_warc_run'] = diag

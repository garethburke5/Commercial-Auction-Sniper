...
Accepts files already supplied to the application; no provider crawling lives here.
"""
...
SUPPORTED={".pdf",".docx",".txt"};LEGACY={".doc"}
@dataclass
class IngestIssue: filename:str;code:str;message:str;severity:str="warning"
@dataclass
class IngestResult:
 documents:list[PackDocument]=field(default_factory=list);issues:list[IngestIssue]=field(default_factory=list);duplicates:list[str]=field(default_factory=list);total_bytes:int=0;total_pages:int=0
...
def locate_evidence(metadata,needle):
 """Locate an extracted excerpt back to its PDF page or DOCX paragraph/table row."""
...
def ingest_bytes(filename,raw):
 ...
 if ext in LEGACY:raise ValueError("Legacy .doc files require conversion to .docx or PDF before analysis; the file has not been silently skipped.")
 if ext not in SUPPORTED:raise ValueError(f"Unsupported legal-pack file type: {ext or 'none'}")
...
def ingest_pack(files:Iterable[tuple[str,bytes]]):
 ...
 for filename,raw in files:
  result.total_bytes+=len(raw);digest=hashlib.sha256(raw).hexdigest()
  if digest in seen:result.duplicates.append(filename);continue
  seen.add(digest)
  try:
   d=ingest_bytes(filename,raw);result.documents.append(d);result.total_pages+=int(d.metadata.get("page_count",0) or 0)
   if d.metadata.get("text_empty"):result.issues.append(IngestIssue(filename,"NO_EXTRACTABLE_TEXT","No machine-readable text was extracted. This may be a scanned document and needs a separate visual/OCR review path."))
  except ValueError as e:result.issues.append(IngestIssue(filename,"LEGACY_DOC" if Path(filename).suffix.lower()==".doc" else "UNSUPPORTED_TYPE",str(e)))

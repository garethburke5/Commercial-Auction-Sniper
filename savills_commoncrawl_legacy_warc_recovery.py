...
def query_index(idx,prefix):
 # Prefix matching avoids guessing any Auc identifier, query order, port spelling, or trailing parameters.
 params={'url':prefix,'output':'json','matchType':'prefix','filter':'status:200','filter':'mime:text/html','collapse':'digest'}
 try:
  r=requests.get(idx['cdx-api'],params=params,headers={'User-Agent':UA},timeout=(6,25))
...
def fetch_warc(row):
 ...
 try:
  ...
  r=requests.get('https://data.commoncrawl.org/'+fn,headers={'User-Agent':UA,'Range':f'bytes={a}-{b}'},timeout=(8,35))
...
def main():
 ...
 idxs=indexes(); discoveries=[]; errors=[]; seen=set(); jobs=[]
